...
def obtain_metadata(response):
    ...
    for sub in oids_sub:  # Using set to obtain unique values
        ...
        for triple in filtered_results:
            ...
            if predicate == 'https://www.w3.org/2019/wot/td#description': #description
                dataset_data['notes'] = object_value
                #logging.error("Domain:"+str(dataset_data['notes']))
    ...
    if response_data is not None:
        ...
        for message in messages:
            ...
            if oids_data is not None:
                ...
                PREFIX adp: <https://auroral.iot.linkeddata.es/def/adapters#> 
                SELECT distinct ?p ?o ?sub WHERE { 
                ?sub rdf:type adp:Dataset .
                ?sub ?p ?o .}
                '''
...
def delete_dataset_CKAN(dataset_name, confirmation_count):
    """
...
def load_counter():
    try:
        with open('counter.yaml', 'r') as file:
...
def update_datasets(commId):
    ...
    logging.error("Inside update_datasets")
    ...
    for dataset_name in existing_datasets:
        if dataset_name not in current_datasets:
            ...
            # Reset the counter if the dataset reappears
...
def get_ckan_datasets():
    ...
    """
    # Create a CKAN API client
    logging.error("inside get_ckan_datasets")
...
def executeUpdate():
    ...
    #logging.error('The community id is:'+str(commId))
